...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        # get all the apps from IdentifiedUser
        app_names = IdentifiedUser.objects.values_list('app_name', flat=True).distinct()
        for app_name in app_names:

            users = IdentifiedUser.objects.filter(app_name=app_name, email__isnull=True, name__isnull=True, phone__isnull=True)
            # remove the users that have already been updated with shopify_update filed as True
            users = users.exclude(shopify_update=True)
            initial_count = users.count()
            updated_count = 0
            skipped_count = 0

            self.stdout.write(f"Starting to update details for {initial_count} users. for app: {app_name}")
            ...
            for user in users:
                ...
                url = f"https://almeapp.co.in/mapCustomer?shop_name={app_name}&customer_id={user.registered_user_id}"
