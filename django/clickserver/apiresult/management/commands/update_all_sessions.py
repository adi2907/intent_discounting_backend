    ...
    def handle(self, *args, **options):
        ...
        for page_number in paginator.page_range:
            ...
            for user in users_to_update:
                ...
                purchase_history = previous_4_sessions.values_list('has_purchased', flat=True)
                user_updates[user.id] = {
                    'purchase_last_4_sessions': 1 if sum(purchase_history) > 0 else 0,
                    'carted_last_4_sessions': 1 if previous_4_sessions.filter(has_carted=True).exists() else 0,
